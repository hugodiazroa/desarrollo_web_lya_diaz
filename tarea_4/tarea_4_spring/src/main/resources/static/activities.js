...
function renderResult(activity, term) {
  ...
  const highlightedDay = highlightMatches(activity.day, term);
  ...
  return `
    <article class="result-card">
      ...
      <p><strong>Day:</strong> ${highlightedDay}</p>
...
function renderEmpty() {
  resultsContainer.innerHTML = '<p class="status-message">No matching activities found.</p>';
}
  ...
  fetch(`/api/activities/search?q=${encodeURIComponent(trimmed)}`)
    ...
    .then(items => {
      ...
      resultsContainer.innerHTML = items
        .map(item => renderResult(item, trimmed))
        .join('');
    })
    .catch(() => {
      resultsContainer.innerHTML = '<p class="status-message">Unable to load search results. Please try again later.</p>';
    });
}
